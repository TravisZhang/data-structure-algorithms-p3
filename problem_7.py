...
class RouteTrieNode:
    def __init__(self, handler = None):
        ...
        self.children = {} # dic to contain (path_block, node) pairs
        self.handler = handler # actually we are using a string
...
class Router:
    ...
    def split_path(self, path):
        ...
        first_flag = 0
        for char in path:
            if char == "/" and first_flag == 1:
                path_list.append(path_block)
                path_block = ""
            elif char != "/":
                path_block += char
            first_flag = 1
